      ...
      <div className="ec-container">
        ...
        <section style={{ padding: '56px 0 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', marginBottom: '32px' }}>
            ...
            <h2 style={{ fontFamily: 'var(--ec-font-body)', fontSize: '15px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--ec-color-text-primary)', margin: 0 }}>
              Our Outlets
            </h2>
            <div style={{ height: '1px', flex: 1, maxWidth: '80px', background: 'var(--ec-color-accent)' }} />
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-8 px-2 lg:px-48 mb-12">
            {outlets.map((outlet, idx) => (
              <div key={idx} style={{ overflow: 'hidden', borderRadius: '4px', border: '1px solid rgba(0,0,0,0.08)', background: 'var(--ec-color-card-bg)', maxWidth: '320px', margin: '0 auto' }}>
                ...
                <div style={{ padding: '8px', borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                  <h3 style={{ fontFamily: 'var(--ec-font-body)', fontSize: '12px', fontWeight: 700, color: 'var(--ec-color-text-primary)', margin: 0, textAlign: 'center', letterSpacing: '0.04em' }}>
                    {outlet.name}
                  </h3>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12" style={{ borderTop: '1px solid rgba(0,0,0,0.08)', paddingTop: '48px' }}>
          ...
          <div>
            <Link href="/e-commerce" style={{ display: 'inline-block', marginBottom: '16px', textDecoration: 'none' }}>
              <span style={{ fontFamily: 'var(--ec-font-body)', fontSize: '24px', fontWeight: 800, letterSpacing: '0.05em', color: 'var(--ec-color-text-primary)' }}>
                ERRUM
              </span>
            </Link>
            <p style={{ fontSize: '13px', lineHeight: 1.7, color: '#555555', maxWidth: '300px', fontFamily: 'var(--ec-font-body)', marginBottom: '24px' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              {[
                ...
                <div key={title} style={{ paddingBottom: '12px', borderBottom: '1px solid rgba(0,0,0,0.06)', marginBottom: '12px' }}>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ec-color-text-primary)', margin: '0 0 2px 0', fontFamily: 'var(--ec-font-body)' }}>{title}</p>
                  <p style={{ fontSize: '12px', color: '#555555', margin: 0, fontFamily: 'var(--ec-font-body)' }}>{sub}</p>
                </div>
              ))}
            </div>

            <a href="https://wa.me/8801942565664" target="_blank" rel="noreferrer"
              style={{ padding: '10px 12px', background: 'var(--ec-color-card-bg)', borderRadius: '4px', border: '1px solid rgba(37,211,102,0.25)', display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}
            >
              <MessageCircle style={{ width: '14px', height: '14px', color: '#25D366', flexShrink: 0 }} />
              <div>
                <p style={{ fontSize: '10px', fontWeight: 700, color: '#999999', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 2px 0', fontFamily: 'var(--ec-font-body)' }}>International Orders</p>
                <p style={{ fontSize: '12px', color: 'var(--ec-color-text-primary)', margin: 0, fontFamily: 'var(--ec-font-body)' }}>WhatsApp: <strong>01942565664</strong></p>
              </div>
            </a>
          </div>
        </div>
        ...
        <div style={{ marginTop: '48px', borderTop: '1px solid rgba(0,0,0,0.08)', paddingTop: '48px' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
            {stores.map(store => (
              <div key={store.name} style={{ padding: '16px', background: 'var(--ec-color-card-bg)', borderRadius: '4px', border: '1px solid rgba(0,0,0,0.08)' }}>
                <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--ec-color-text-primary)', margin: '0 0 8px 0', fontFamily: 'var(--ec-font-body)' }}>{store.name}</p>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '8px' }}>
                  <MapPin style={{ width: '14px', height: '14px', color: '#999999', marginTop: '2px', flexShrink: 0 }} />
                  <span style={{ fontSize: '12px', color: '#555555', fontFamily: 'var(--ec-font-body)', lineHeight: 1.5 }}>{store.address}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Phone style={{ width: '14px', height: '14px', color: '#999999', flexShrink: 0 }} />
                  <a href={`tel:${store.phone}`} style={{ fontSize: '12px', color: '#555555', fontFamily: 'var(--ec-font-body)', textDecoration: 'none' }}>{store.phone}</a>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginTop: '48px', paddingTop: '24px', borderTop: '1px solid rgba(0,0,0,0.08)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          <p style={{ fontSize: '12px', color: '#999999', fontFamily: 'var(--ec-font-body)', margin: 0 }}>
