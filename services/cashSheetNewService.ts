...
const cashSheetNewService = {
  async getSheet(month: string): Promise<CashSheetResponse> {
    const res = await axiosInstance.get('/cash-sheet-new', { params: { month, _ts: Date.now() } });
    return res.data;
  },

  async getEntries(date: string): Promise<DayEntries> {
    const res = await axiosInstance.get('/cash-sheet-new/entries', { params: { date, _ts: Date.now() } });
    return {
      ...res.data,
  ...
  async addBranchCost(payload: { entry_date: string; store_id: number; amount: number; details?: string }): Promise<BranchCostEntry> {
    const res = await axiosInstance.post('/cash-sheet-new/branch-cost', payload);
    return normalizeBranchCostEntry(res.data.entry);
  },
  async deleteBranchCost(id: number): Promise<void> {
    await axiosInstance.delete(`/cash-sheet-new/branch-cost/${id}`);
  },
