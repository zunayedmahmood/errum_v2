...
function companyInfoBlock(r?: any) {
  const name = r?.storeName || 'Errum BD';
  const address = r?.storeAddress || 'Level 03, Lift 2, Haji Kujrot Ali Mollah Super Market, Dhaka 1216';
  const phone = r?.storePhone || '01942-565664';

  return `
    <div class="company">
      <div class="name">${escapeHtml(name)}</div>
      ${address ? `<div class="line">${escapeHtml(address)}</div>` : ''}
      ${phone ? `<div class="line">Mobile: ${escapeHtml(phone)}</div>` : ''}
    </div>
  `;
}
