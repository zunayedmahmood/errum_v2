...
import checkoutService, { Address, OrderItem, PaymentMethod, DeliveryChargeSettings } from '@/services/checkoutService';
...
export default function CheckoutClient() {
  ...
  const { customer: authenticatedCustomer } = useCustomerAuth();
  ...
  const [shippingCharge, setShippingCharge] = useState(checkoutService.calculateDeliveryCharge('Dhaka'));
  const [deliveryChargeSettings, setDeliveryChargeSettings] = useState<DeliveryChargeSettings>(checkoutService.getCachedDeliveryChargeSettings());
  ...
  // Load admin-configured e-commerce delivery charges before totals are shown.
  useEffect(() => {
    let isMounted = true;

    checkoutService.getDeliveryChargeSettings()
      .then((settings) => {
        if (!isMounted) return;
        setDeliveryChargeSettings(settings);
        const activeCity = isGuestCheckout()
          ? (guestAddress.city || 'Dhaka')
          : (addresses.find(a => a.id === selectedShippingAddressId)?.city || addressForm.city || 'Dhaka');
        setShippingCharge(checkoutService.calculateDeliveryCharge(activeCity));
      })
      .catch(() => {
        // checkoutService already falls back to defaults; keep checkout usable.
      });
  ...
  }, [selectedShippingAddressId, addresses, deliveryChargeSettings]);
  ...
  }, [guestAddress.city, deliveryChargeSettings]);



  // While creating/editing an address, immediately preview the charge for the typed city.
  useEffect(() => {
    if (!isGuestCheckout() && showAddressForm) {
      setShippingCharge(checkoutService.calculateDeliveryCharge(addressForm.city || 'Dhaka'));
    }
  }, [addressForm.city, showAddressForm, deliveryChargeSettings]);
  ...
  const loyaltyEligibleAmount = Math.max(0, summary.subtotal - summary.discount_amount);
  const selectedShippingAddress = addresses.find((address) => address.id === selectedShippingAddressId);
  const loyaltyPhone = isGuestCheckout()
    ? guestPhone
    : (authenticatedCustomer?.phone || selectedShippingAddress?.phone || addressForm.phone || '');
  const loyaltyDiscount = useLoyaltyPoints
    ? Math.min(loyaltyEligibleAmount, Number(loyaltyPreview?.redeemable_taka || 0))
    : 0;
  const checkoutTotal = Math.max(0, summary.total_amount - loyaltyDiscount);
    ...
    try {
      ...
      if (paymentUrl) {
        ...
        try {
          ...
          const amt = resp?.data?.total_amount ?? checkoutTotal;
          if (orderNo) {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (isOnlinePayment && checkoutTotal > 0) {
      ...
      <div className="ec-root ec-darkify min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8">
          ...
          <SSLCommerzPayment
            ...
            loyaltyExpectedPoints={useLoyaltyPoints ? Number(loyaltyPreview?.points_to_redeem || 0) : undefined}
            loyaltyExpectedDiscount={useLoyaltyPoints ? Number(loyaltyPreview?.redeemable_taka || 0) : undefined}
            onError={(error) => {
              ...
              <div className="bg-[var(--bg-surface)] rounded-[var(--radius-lg)] border border-[var(--border-default)] p-6 sticky top-24 shadow-sm">
                ...
                <div className="border-t border-[var(--border-default)] mt-6 pt-6 space-y-3">
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--border-strong)] mt-4 pt-4">
                    ...
                    <span className="text-2xl font-bold text-[var(--text-primary)]" style={{ fontFamily: "'Poppins', sans-serif" }}>৳{checkoutTotal.toLocaleString()}</span>
                  </div>
                </div>
              ...
              <div className="bg-[var(--bg-surface)] rounded-[var(--radius-lg)] border border-[var(--border-default)] overflow-hidden shadow-sm">
                ...
                >
                  ...
                  <div className="flex items-center gap-3 sm:hidden">
                    <span className="text-lg font-bold text-[var(--text-primary)]">৳{checkoutTotal.toLocaleString()}</span>
                    ...
                    <div className="space-y-3 pt-6">
                      ...
                      {loyaltyDiscount > 0 && (
                        <div className="flex justify-between text-sm text-violet-600">
                          ...
                          <span className="font-bold">- ৳{loyaltyDiscount.toLocaleString()}</span>
                        </div>
                      )}
