...
const fmt = (n: number) => {
  ...
  return `${value < 0 ? '-' : ''}৳${Math.abs(value).toLocaleString('en-BD', { maximumFractionDigits: 2 })}`;
};
...
function dayLabel(dateStr: string) {
  ...
  return d.toLocaleDateString('en-BD', { timeZone: 'Asia/Dhaka', day: '2-digit', weekday: 'short' });
}
...
function StatCell({ value, highlight }: { value: number; highlight?: 'green' | 'blue' | 'amber' }) {
  const color =
    value < 0 ? 'text-rose-600 dark:text-rose-400 font-semibold' :
...
export default function CashSheetPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await cashSheetService.getSheet(month, isBranch && userStoreId ? Number(userStoreId) : null);
      setRows(res.data);
  ...
  const visibleStores = isBranch && userStoreId
    ? stores.filter(s => s.id === Number(userStoreId))
    : stores;
          ...
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {isAdmin ? 'All branches + online channel · live rebalancing by selected sale date' : `Branch: ${visibleStores.map(s => s.name).join(', ')}`}
              </p>
            </div>
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
              <table className="text-xs border-collapse min-w-max bg-white dark:bg-gray-900">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {rows.map((row, idx) => {
                    const isToday = row.date === todayDhaka();
                    const rowBg = isToday
                    ...
                    <tr className="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-600 font-semibold">
                      ...
                      {visibleStores.map(s => {
                        ...
                        return (<Fragment key={s.id}>
                          ...
                          <td className="px-2 py-2 text-right text-xs tabular-nums text-gray-800 dark:text-gray-200">{fmt(b?.cash_to_bank ?? 0)}</td>
                        </Fragment>);
                      })}
