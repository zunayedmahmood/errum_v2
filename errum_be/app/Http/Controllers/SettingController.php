...
class SettingController extends Controller
{
    ...
    /**
     * Get e-commerce delivery charge settings for public checkout and admin panel.
     */
    public function getDeliveryChargeSettings(DeliveryChargeService $deliveryChargeService)
    {
        return response()->json([
            'success' => true,
            'data' => $deliveryChargeService->getSettings(),
        ]);
    }

    /**
     * Update e-commerce delivery charges from the admin settings page.
     */
    public function updateDeliveryChargeSettings(Request $request, DeliveryChargeService $deliveryChargeService)
    {
        $validated = $request->validate([
            'inside_dhaka' => ['required', 'numeric', 'min:0', 'max:100000'],
            'outside_dhaka' => ['required', 'numeric', 'min:0', 'max:100000'],
        ]);

        $settings = $deliveryChargeService->updateSettings($validated);
