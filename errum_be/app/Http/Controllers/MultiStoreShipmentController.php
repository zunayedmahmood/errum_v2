...
class MultiStoreShipmentController extends Controller
{
            ...
            foreach ($itemsByStore as $storeId => $items) {
                ...
                try {
                    ...
                    $shipment = Shipment::create([
                        ...
                        'item_quantity' => $items->sum('quantity'),
                        'item_weight' => $itemWeight,
                        'amount_to_collect' => $pathaoData['amount_to_collect'],
                        'metadata' => [
    ...
    public function trackAllShipments($orderId): JsonResponse
    {
        try {
            ...
            foreach ($order->shipments as $shipment) {
                ...
                try {
                    $tracking = $this->trackPathaoShipment(
                        $shipment->pathao_consignment_id,
                        $shipment->store->pathao_key
                    );
     ...
     */
    private function createPathaoShipment(array $data, ?string $accessToken): array
    {
        try {
            if (!$accessToken) {
                return [
                    'success' => false,
                    'message' => 'Store does not have Pathao access token configured',
                ];
            }
            ...
            ])->post('https://api-hermes.pathao.com/api/v1/orders', $data);
     ...
     */
    private function trackPathaoShipment(string $consignmentId, ?string $accessToken): array
    {
        try {
            if (!$accessToken) {
                throw new \Exception('No Pathao access token');
            }

            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $accessToken,
                'Accept' => 'application/json',
            ])->get("https://api-hermes.pathao.com/api/v1/orders/{$consignmentId}");
